...
async function getCroppedImg(imageSrc: string, _crop: { x: number; y: number }, _zoom: number, _aspect: number, pixelCrop: any): Promise<Blob> {
...
const ImageCropUpload: React.FC<ImageCropUploadProps> = ({ value = [], onChange, maxCount = 5 }) => {
  ...
  const handleCropOk = async () => {
    ...
    try {
      const blob = await getCroppedImg(imageSrc, crop, zoom, 1, croppedAreaPixels);
      const fileName = `cropped-${Date.now()}.png`;
      const croppedFile = new File([blob], fileName, { type: 'image/png' });
      const newItem = { url: URL.createObjectURL(croppedFile), file: croppedFile };
  ...
  return (
    <div className="space-y-2">
      <Upload
        ...
        fileList={value.map((v, idx) => ({ uid: String(idx), name: `image-${idx+1}.png`, status: 'done', url: v.url }))}
        beforeUpload={beforeUpload}
        onRemove={(file) => {
          const index = value.findIndex(v => v.url === file.url);
      ...
      >
        {value.length >= maxCount ? null : <div><UploadOutlined /> <div style={{ marginTop: 8 }}>Rasm qo'shish</div></div>}
      </Upload>
      <Modal
        title="Rasmni kesish"
        open={cropModalOpen}
